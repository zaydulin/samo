		...
		if ( !isNoviBuilder && !isIE && !isMobile ) {
			...
			$window.on('scroll', function () {
				for ( let i = 0; i < plugins.rdParallax.length; i++ ) {
					...
					let prevElement = $parallax.prev();
					let nextElement = $parallax.next();
					...
					if (prevElement.length) {
						...
						if (!prevElement.attr('class').split(/\s+/).some(cls => cls.startsWith('bg-'))) {
							prevElement.addClass('bg-default');
						}
					}
					...
					if (nextElement.length) {
						...
						if (!nextElement.attr('class').split(/\s+/).some(cls => cls.startsWith('bg-'))) {
							nextElement.addClass('bg-default');
						}
					}
